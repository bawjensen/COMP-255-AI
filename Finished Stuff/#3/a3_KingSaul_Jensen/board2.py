...
"""			Attempted implementation of double jumping.


			# Double-Jumping from end
			no_jumps_more = False
			new_start = end
			while not no_jumps_more:
				for new_end in Board.jumps_by_c_num[new_start]:
					if self.c_board[new_end] == Board.EMPTY_SLOT:
						jump_direction = Board.direction_from_coords[(new_start, new_end)]
						for pos_end in Board.moves_by_c_num[new_start]:
							if Board.direction_from_coords[(new_start, pos_end)] == jump_direction:
								doub_jumpable = True
								break
